...
{
	//
    // Algorithme de placement des notes
	//
	int i;
	notes[0].stringIdx = 2;
	for (i=1; i<nbNotes; i++)
	{
		if(notes[i].value > notes[i-1].value)
		{
			...
			if (notes[i].stringIdx > 4) // Vérification si on ne depasse pas le nombre de cordes
			{
				notes[i].stringIdx = 4;
			}
		}
		...
		{
			notes[i].stringIdx = notes[i-1].stringIdx - 1;
			if (notes[i].stringIdx < 0) // Vérification si on ne depasse pas le nombre de cordes
			{
				notes[i].stringIdx = 0;
			}
		}
