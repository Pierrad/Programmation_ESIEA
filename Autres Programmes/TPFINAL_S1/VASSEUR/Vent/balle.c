...
Balle chargerBalle(char* chemin)
{
    ...
    if (fichier != NULL ) // On verifie que le pointeur sur fichier est valide
    {
        ...
        &info.masse, &info.coeffriction, &info.position.x, &info.position.y, &info.vitesse.x, &info.vitesse.y, &info.vent.x, &info.vitesse.y); // Remplissage de la structure
        fclose(fichier);
    }
    else
...
int majPosition(Balle * balle, float dt) // Met a jour la position de la balle avec le PFD
{
    ...
    Vecteur fvent = multScalVect(balle->coeffriction, balle->vent); // Calcul de la force du vent
    ...
    Vecteur somme = addVect(addVect(poids, frottements), fvent); // Sommes des forces (poids, frottements et vent)
