...
Balle chargerBalle(char* chemin){
    ...
    if (fichier != NULL ) // On verifie que le pointeur sur fichier est valide 
    {
        ...
        &info.masse, &info.coeffriction, &info.position.x, &info.position.y, &info.vitesse.x, &info.vitesse.y, &info.attenuation, %info.attracteurs); // Remplissage de la structure
        fclose(fichier);
    }
