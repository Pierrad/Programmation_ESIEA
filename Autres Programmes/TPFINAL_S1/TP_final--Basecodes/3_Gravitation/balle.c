...
int majPosition(Balle * balle, AttracteurList *pAttList, float dt) // Met a jour la position de la balle avec le PFD en fonction des attracteurs
{
    ...
    { 
        ...
        float distance = pow(normVect(position), 2), magnitude; // Calcule du carre de la distance qui intervient dans la formule de la force 
        if (distance!=0)
        {
            magnitude = (-0.1) / distance; // Calcul de la partie flottante de la formule de la force
